 ...
 * 访问后未登录会跳转到智慧职教统一认证 sso.icve.com.cn/sso/auth_v2（redirect 指向 zjy2 /admin/pedding）。
 */
const LOGIN_URL = 'https://zjy2.icve.com.cn/study/index';

/**
 * 职教云登录页（即 sso.icve.com.cn 统一认证，与 icve 同源，已通过 Playwright MCP 实测）：
 * - 登录页默认是「手机验证码登录」tab，需点击 .tabs span:nth-of-type(2)（账号密码登录）切换
 * - 表单：input[placeholder="请输入账号/密码"]、.agreement label.el-checkbox、.demo-ruleForm .login
 * - 未勾选协议点登录会先弹 .el-message-box（协议确认框），checkAgreement 幂等勾选可规避
 * - 弹窗：#aliyunCaptcha-window-popup（class=window-show / display:block 时可见）
 * - 背景图：#aliyunCaptcha-img（实测为 data URL，自然宽 296 显示 300）
 * - 拼图：#aliyunCaptcha-puzzle（实测为 data URL，显示宽 52，初始 left:0 与背景图左缘对齐）
 * - 滑块：#aliyunCaptcha-sliding-slider（40x40，轨道 300px，初始位于轨道左端）
 * - 旧代码 .xcConfirm 选择器已失效（实测页面不存在），错误弹窗为 Element UI .el-dialog
 */
const SEL = {
	username: 'input[placeholder="请输入账号"]',
	password: 'input[placeholder="请输入密码"]',
	/** 登录方式 tab：第 2 个为「账号密码登录」（默认是手机验证码登录） */
	tabPassword: '.tabs span:nth-of-type(2)',
	agreement: '.agreement label.el-checkbox',
	submit: '.demo-ruleForm .login',
...
/** 拼图图形在 shadow.png 内的左边距（与 icve 同源 SDK，恒约 2px；跨域/DataURL 图无法 canvas 分析，故用常量） */
const PIECE_PAD = 2;
		...
		async run(page, configs, options?: OCROptions) {
			try {
				...
				if (options?.ocrApiUrl && options?.detTargetKey && options?.detBackgroundKey) {
					...
					if (err) ZJYLoginScript.emit('script-error', err);
				}
			} catch (err) {
				ZJYLoginScript.emit('script-error', getErrorMessage(err));
			}
		}
	}
);
...
/**
 * 访问职教云首页后若 URL 仍包含 sso/auth，视为未登录。
 * 登录页默认渲染手机验证码表单，故等待 .demo-ruleForm（表单容器）作为页面就绪标志。
 */
async function isNotLogin(page: Page): Promise<boolean> {
	await page.goto(LOGIN_URL, { waitUntil: 'domcontentloaded' });
	await page.waitForSelector('.demo-ruleForm', { timeout: 15000 }).catch(() => {});
	return page.url().includes('sso/auth');
}

/** 切换到「账号密码登录」tab（幂等：账号输入框已可见则跳过） */
async function switchToPasswordTab(page: Page): Promise<void> {
...
/**
 * 滑块验证：
 * 1. 取背景图与拼图原图（toBase64：data URL 直接截取，http(s) URL 用 Node 侧下载）
...
/**
 * 读取可见的错误弹窗文本（滑块验证通过后服务端校验账号/密码失败时弹出，
 * 如"账号不存在，请确认后登录或者注册！"）。
 * 兼容 Element UI 的 .el-dialog 与 .el-message-box（过滤协议确认框等非错误内容）。
